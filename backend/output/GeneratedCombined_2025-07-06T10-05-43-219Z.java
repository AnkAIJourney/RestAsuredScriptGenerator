...
public class AWS3_Reports_GetXML {
    ...
    public ValidatableResponse getXmlWithBenchmarkId(ValidatableResponse jwtToken, String payLoad) {
        return getXml_APITest_Internal(LoginCookies.MSDEM2Path, jwtToken, payLoad);
    }

    public ValidatableResponse getXmlWithReportFlags(ValidatableResponse jwtToken, String payLoad) {
        return getXml_APITest_Internal(LoginCookies.MSDEM2Path, jwtToken, payLoad);
    }
    ...
    public ValidatableResponse getXmlWithoutCookies(ValidatableResponse jwtToken, String payLoad) {
        return getXml_APITest_InternalWithoutCookies(jwtToken, payLoad);
    }
    ...
    private ValidatableResponse getXml_APITest_InternalWithoutCookies(ValidatableResponse jwtToken, String xmlBody) {
        RequestSpecBuilder builder = new RequestSpecBuilder();
        JsonPath jsonPathEvaluator = jwtToken.extract().jsonPath();
        String jwtTokenResponse = jsonPathEvaluator.get("jwtToken");
        String endPoint = CacheManager.GlobalDict.get("AWSIpxReportEndpoint");
...
public class AWS3_IPX_Reports extends AbstractTest {
    ...
    public void verifyBenchmarkId_AutoAsset() throws IOException {
        ...
        ValidatableResponse response = api.getXmlWithBenchmarkId(jwtToken, jsonContent);
    ...
    public void verifyReportsBasedOnReportFlag_0() throws IOException {
        ...
        ValidatableResponse jwtToken = jwtTokenAPI.jwtTokenWithValidRoles();
        ValidatableResponse response = api.getXmlWithReportFlags(jwtToken, jsonContent);
        response.statusCode(200);
        String responseBody = response.extract().asString();
    ...
    public void verifyReportsBasedOnReportFlag_Mixed() throws IOException {
        ...
        String responseBody = response.extract().asString();
        sAssert.assertFalse(responseBody.contains("<xray"), "xray should not be present");
        sAssert.assertFalse(responseBody.contains("<IncludeDisc"), "IncludeDisc should not be present");
        sAssert.assertFalse(responseBody.contains("<InvestDetail"), "InvestDetail should not be present");
        sAssert.assertTrue(responseBody.contains("<snapshot"), "snapshot should be present");
        sAssert.assertFalse(responseBody.contains("<SingleFee"), "SingleFee should not be present");
        sAssert.assertTrue(responseBody.contains("<portcomprpt"), "portcomprpt should be present");
        sAssert.assertFalse(responseBody.contains("<StkIntersect"), "StkIntersect should not be present");
        sAssert.assertFalse(responseBody.contains("<portfolioFee"), "portfolioFee should not be present");
        sAssert.assertTrue(responseBody.contains("<IncludeFeeComp"), "IncludeFeeComp should be present");
        sAssert.assertAll();
    }

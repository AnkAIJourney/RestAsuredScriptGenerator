...
public void Test_4() throws IOException {
    ...
    String xmlFilePath = "src\\test\\resources\\testData\\RequestData\\ipx\\generateReport.xml";
    String xmlInput = new String(Files.readAllBytes(Paths.get(xmlFilePath)), StandardCharsets.UTF_8);
    ValidatableResponse response = api.generateReport_APITestWithoutToken(xmlInput);
...
public void Test_6() throws IOException {
    ...
    response.statusCode(500);
    sAssert.assertTrue(response.extract().asString().contains("Internal Server Error: File not found"), "Error message for incorrect assetClassGoupId is incorrect");
...
public void Test_10() throws IOException {
    ...
    ValidatableResponse response = api.generateReport_APITest(jwtToken, "{}");
    response.statusCode(400);
    sAssert.assertTrue(response.extract().asString().contains("A non-empty request body is required."), "Error message for empty JSON body is incorrect");
    sAssert.assertAll();
}
```
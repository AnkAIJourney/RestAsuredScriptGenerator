...
public class AWS3_IPX_Reports extends AbstractTest {
    ...
    public void verifyBenchmarkId_AutoAsset() throws IOException {
        ...
        jsonContent = jsonContent.replace("currentBenchmarkIdValue", "AutoAsset").replace("proposedBenchmarkIdValue", "AutoAsset");
    ...
    public void verifyBenchmarkId_dcc56cad_a24f_4d93_9522_86435029d494() throws IOException {
        ...
        String jsonFilePath = "src\\test\\resources\\testData\\RequestData\\ipx\\getXml.json";
        String jsonContent = new String(Files.readAllBytes(Paths.get(jsonFilePath)));
        jsonContent = jsonContent.replace("currentBenchmarkIdValue", "dcc56cad-a24f-4d93-9522-86435029d494").replace("proposedBenchmarkIdValue", "dcc56cad-a24f-4d93-9522-86435029d494");
        ValidatableResponse jwtToken = jwtTokenAPI.jwtTokenWithValidRoles();
        ValidatableResponse response = api.getXml_APITest(jwtToken, jsonContent);
        response.statusCode(200);
        String responseBody = response.extract().asString();
    ...
    public void verifyBenchmarkId_FOUSA06WNW() throws IOException {
        ...
        String jsonFilePath = "src\\test\\resources\\testData\\RequestData\\ipx\\getXml.json";
        String jsonContent = new String(Files.readAllBytes(Paths.get(jsonFilePath)));
        jsonContent = jsonContent.replace("currentBenchmarkIdValue", "FOUSA06WNW").replace("proposedBenchmarkIdValue", "FOUSA06WNW");
        ValidatableResponse jwtToken = jwtTokenAPI.jwtTokenWithValidRoles();
        ValidatableResponse response = api.getXml_APITest(jwtToken, jsonContent);
        response.statusCode(200);
        String responseBody = response.extract().asString();
    ...
    public void verifyBenchmarkId_Invalid() throws IOException {
        ...
        String jsonFilePath = "src\\test\\resources\\testData\\RequestData\\ipx\\getXml.json";
        String jsonContent = new String(Files.readAllBytes(Paths.get(jsonFilePath)));
        jsonContent = jsonContent.replace("currentBenchmarkIdValue", "&9r89rnnkldf").replace("proposedBenchmarkIdValue", "&9r89rnnkldf");
        ValidatableResponse jwtToken = jwtTokenAPI.jwtTokenWithValidRoles();
        ValidatableResponse response = api.getXml_APITest(jwtToken, jsonContent);
        response.statusCode(200);
        String responseBody = response.extract().asString();
    ...
    public void verifyReportsBasedOnReportFlag_Mixed() throws IOException {
        ...
        response.statusCode(200);
        String responseBody = response.extract().asString();
        sAssert.assertFalse(responseBody.contains("<IncludeDisc"), "IncludeDisc element should not be present");
        sAssert.assertFalse(responseBody.contains("<InvestDetail"), "InvestDetail element should not be present");
        sAssert.assertTrue(responseBody.contains("<SnapShot"), "SnapShot element should be present");
        sAssert.assertFalse(responseBody.contains("<SingleFee"), "SingleFee element should not be present");
        sAssert.assertTrue(responseBody.contains("<portfolioComparision"), "portfolioComparision element should be present");
        sAssert.assertFalse(responseBody.contains("<StkIntersect"), "StkIntersect element should not be present");
        sAssert.assertFalse(responseBody.contains("<portfolioFee"), "portfolioFee element should not be present");
        sAssert.assertTrue(responseBody.contains("<IncludeFeeComp"), "IncludeFeeComp element should be present");
        sAssert.assertAll();
    }
